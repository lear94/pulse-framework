//! Cuarentena de Actix-web. TODO el acoplamiento al framework HTTP vive aquí:
//! el adaptador (`api`: handlers, extractores, mapeo de errores) y el arranque
//! del servidor (`bootstrap`: `HttpServer`/`Cors`/middleware). El motor
//! (services/persistence/auth/core/store/state/pulse) NO depende de este módulo.
//! Cambiar de framework web (Axum, etc.) = reescribir solo `src/web/`.
